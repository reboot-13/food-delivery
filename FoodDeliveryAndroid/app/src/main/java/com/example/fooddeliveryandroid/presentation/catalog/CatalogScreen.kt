...
import com.example.fooddeliveryandroid.presentation.AppSnackBar
...
fun CatalogScreen (
    ...
    val events = catalogViewModel.events
...
fun CatalogSnackBars(
    ...
    LaunchedEffect(Unit) {
        events.collect { event ->
            when (event) {
                is CatalogEvent.ShowAuthSnackBar -> {
                    val result = snackBarHostState.showSnackbar(
                        message = "Чтобы добавить товар в корзину, необходимо авторизоваться",
                        actionLabel = "Войти",
                ...
                is CatalogEvent.ShowSyncSnackBar -> {
                    val result = snackBarHostState.showSnackbar(
                        message = "Cервер не отвечает или нет подключения к Интернету",
